...
- Bloqueio entre usuários (#20): actions `bloquear/desbloquear/estaBloqueado/listarIdsBloqueados` (`src/lib/bloqueio/actions.ts`), botão no perfil de outro usuário, ocultação de comentários de bloqueados no mural e nas histórias, e impedimento de quem foi bloqueado postar no mural do dono (RPC `existe_bloqueio`, migration 025). Tabela `bloqueio` já tinha RLS.
- Notificações em tempo real (#19): `notificacao` adicionada à publicação `supabase_realtime` (migration 024); `NotificacoesPopup` carrega no mount e assina INSERTs via Realtime (RLS respeitada).
- Catálogo com scroll infinito (#16): server action `carregarHistorias` + componente client `CatalogoInfinito` (IntersectionObserver), substituindo a paginação por página.

- Padronização visual (2026-06-29): linguagem consistente aplicada às telas — cards `rounded-2xl` com borda clara e sombra suave, paleta indigo/violeta, acentos com ícones, estados vazios em caixa e botões/inputs uniformes. Telas redesenhadas nesta passada: entrar, cadastro, perfil, detalhe da história, catálogo, home, configurações e favoritos, além do componente `CardHistoria` (corrigido hover azul para indigo). No perfil, os cards de "Histórias escritas" passaram a mostrar capa, nota média e visualizações (`buscarPerfilPublico` retorna esses campos). Telas já consistentes mantidas: biblioteca, projeto/novo, projeto/editar, importar, colaboradores, notificações, editor de escrita e leitura.
...
- Publicação por capítulo: documentos novos agora nascem como `rascunho`/`publico=false`; editor permite mudar para revisão, revisão supervisionada ou publicado; leitura pública filtra somente capítulos publicados; publicação de capítulo notifica favoritos. Bloqueios de publicação/aprovação exibem mensagens públicas específicas na UI.
- Colaboração editorial: a tela de colaboradores foi redesenhada para buscar usuários por nome/usuário, explicar convites, papéis e status de aceite; convites aceitam entrada com `@nome`, evitam duplicidade e deixam claro que pendentes ainda não acessam o projeto. A criação de projeto também explica que colaboradores são opcionais e que o convite depende de aceite. Ao aceitar convite, o colaborador é direcionado para a escrita e passa a ver o projeto na biblioteca com badge de colaboração.
- Notificações: página e popup têm feedback de carregamento/erro, contador de não lidas, atualização manual e aceite de convite com estado em andamento; ações ficam mais claras em telas pequenas.
- Importação: fluxo EPUB/DOCX agora explicita limites de 5MB e 80 capítulos, permite selecionar/desmarcar todos no preview, normaliza títulos vazios/longos e persiste capítulos importados como `rascunho`/`publico=false`.
