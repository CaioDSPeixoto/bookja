...
function Btn({ ativo, onClick, children, title }: { ativo: boolean; onClick: () => void; children: React.ReactNode; title: string }) {
  return (
    <button
      ...
      onClick={onClick}
      title={title}
      className={`flex h-8 w-8 items-center justify-center rounded transition-colors ${ativo ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'}`}
    >
      {children}
    </button>
  )
...
export default function BarraFerramentas({ editor }: Props) {
  ...
  return (
    <div className="flex h-10 items-center gap-0.5 border-b border-gray-200 bg-gray-50/80 px-3">
