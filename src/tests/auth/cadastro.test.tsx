...
function preencherFormulario(email: string, usuario: string, senha: string, confirmar: string) {
  const inputs = document.querySelectorAll('input')
  // ordem: email, nome_usuario, senha, confirmar_senha
  fireEvent.change(inputs[0], { target: { value: email } })
  fireEvent.change(inputs[1], { target: { value: usuario } })
  fireEvent.change(inputs[2], { target: { value: senha } })
  fireEvent.change(inputs[3], { target: { value: confirmar } })
}
...
describe('Página de Cadastro', () => {
  ...
  it('exibe erro quando senhas não coincidem', async () => {
    render(<CadastroPage />)
    preencherFormulario('a@b.com', 'user1', '123456', '654321')
  ...
  it('chama signUp com metadados quando senhas coincidem', async () => {
    ...
    render(<CadastroPage />)
    preencherFormulario('a@b.com', 'user1', '123456', '123456')
    fireEvent.click(screen.getByRole('button', { name: 'Cadastrar' }))
    ...
    await waitFor(() => {
      expect(mockSignUp).toHaveBeenCalledWith({
        email: 'a@b.com',
        password: '123456',
        options: { data: { nome_usuario: 'user1' } },
      })
    })
  })
